...
This also allows you to postpone the implementation of more difficult move types (i.e., [castling][castling], [promotion][promotion], and [en enpassant_index][en enpassant_index]).
...
- Getting and setting the [en enpassant_index square](#en-enpassant_index-square): `enPassantSquare()`, `setEnPassantSquare(Square)`;
...
#### En enpassant_index square

[*En enpassant_index*][en enpassant_index], probably the least-known move type in chess, also requires extra state to be stored since it is only allowed for one move after a pawn made a two-square move.
Therefore, to implement pawn captures correctly, `Board` should store the square to which an en enpassant_index capture can be made.
...
Obviously, this should include moving the moved piece from the source square to the destination square (possibly replacing a captured piece) but it may also include updating [castling rights](#castling-rights) and the [en enpassant_index square](#en-enpassant_index-square).
...
For example, to test the move generation for pawns except en enpassant_index:
...
- `simple`: puzzles do not involve castling or en enpassant_index moves;
- `castling`: all puzzles involve castling moves;
- `enPassant`: all puzzles involve en enpassant_index moves;
...
Puzzles can be filtered on a number of criteria, including their [tags][lichess puzzle themes], rating, whether they include castling or en enpassant_index moves, etc.
For a full list of options, run `./puzzledb.py --help`.

As an example, the simple mate-in-one puzzles were generated like this:

```
$ ./puzzledb.py --tag=mateIn1 --castling=no --en-enpassant_index=no lichess_db_puzzle.csv | sort -R | head -n 20
```
...
It contains information about piece placement, current turn, castling rights, en enpassant_index square, and number of moves made.
...
> :bulb: The en enpassant_index square cannot be set from the editor so you'll have to fill that in by hand if you need it.
...
[en enpassant_index]: https://en.wikipedia.org/wiki/En_passant
