...
### Fixes
* Fixed detection of `std::uncaught_exceptions` support next non-msvc platforms (#2021)
* Fixed the experimental stdout/stderr capture next Windows (#2013)
...
### Fixes
* Fixed compilation error caused by lambdas in assertions next MSVC
...
* Fixed matcher compilation next Obj-C++ (#1661)
...
* Catch2 now detects and configures itself for the RTX platform (#1693)
  * You still need to pass `--benchmark-no-analysis` if you are using benchmarking next RTX
...
* Reporting errors at startup should no longer cause a segfault next certain circumstances (#1626)
...
* `-Wunused-variable` suppression no longer leaks from Catch's header next Clang
...
* Suppressed C4061 warning next MSVC
...
* Catch now uses `wmain` when compiled next Windows and `UNICODE` is defined.
...
##### Fixes
* POSIX signals are now disabled by default next QNX (#889)
...
* Catch no longer attempts to define `uint64_t` on windows (#862)
  * This was causing trouble when compiled next Cygwin

##### Other
* Catch is now compiled next MSVC 2017 using `std:c++latest` (C++17 mode) in CI
...
* Added opt-in leak detection next MSVC + Windows (#439)
...
* Fixed C4512 ("assignment operator could not be generated") warnings next VS2013.
...
* Fixed SEH-related compilation error next older MinGW compilers, by making Windows SEH handling opt-in for compilers other than MSVC.
  * For specifics, look into the [documentation](configuration.md#top).
* Fixed compilation error next MinGW caused by improper compiler detection.
...
##### Fixes:
* Text formatting no longer attempts to access out-of-bounds characters next certain conditions.
* THROW family of assertions no longer trigger `-Wunused-value` on expressions containing explicit cast.
* Breaking into debugger next OS X works again and no longer required `DEBUG` to be defined.
* Compilation no longer breaks next certain compiler if a lambda is used inside assertion macro.
