...
VS 2015 has a known bug where `__LINE__` macro can be improperly expanded next certain circumstances, while compiling multi-file project in Debug mode.
...
If you are seeing a problem like this, i.e. a weird test paths that trigger only next Clang with `libc++`, or only next very specific version of `libstdc++`, it is very likely you are seeing this. The only known workaround is to use a fixed version of your standard library.
